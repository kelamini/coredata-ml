...
from .core.utils.io import get_local_path
...
def get_all_classes_inherited_CoreDataMLBase(script_file):
    names = set()
    ...
    for name, obj in inspect.getmembers(module, inspect.isclass):
        if name == CoreDataMLBase.__name__:
            continue
        if issubclass(obj, CoreDataMLBase):
            names.add(name)
        for base in obj.__bases__:
            if CoreDataMLBase.__name__ == base.__name__:
                names.add(name)
    sys.path.pop()
    return list(names)
